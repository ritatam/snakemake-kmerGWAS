...
parser.add_argument("-p", "--perm", metavar="", required=True, help="Proportion of data to be permuted.")
...
def kmers_to_fasta(Yr_name, date, perm, kmer_fn, k="31"):
    with open(kmer_fn, "r") as kmers:
        kmers.readline()
        kmerlist = [line[2:int(k)+2] for line in kmers.readlines()]
        if not kmerlist:
            with open(os.path.join(args.outdir, f"{date}_analysis", f"perm_{perm}", Yr_name, "empty_kmer.txt"), "w") as empty_flag:
                print(f"{Yr_name}'s k-mer file is empty.", file=empty_flag)
                return
    kmers_fasta_dir = os.path.join(args.outdir, f"{date}_analysis", f"perm_{perm}", Yr_name, "kmers_fasta")
...
if __name__ == "__main__":
    kmers_to_fasta(args.Yr_name, args.date, args.perm, args.kmer_filename, args.kmer_length)
