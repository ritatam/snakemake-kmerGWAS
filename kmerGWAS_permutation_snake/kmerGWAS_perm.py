...
parser.add_argument("-d", "--date", required=True, help="Date as prefix. YYYYMMDD.")
parser.add_argument("-n", "--host_strain_name", required=True, help="Annotations of host strains, e.g. 'Yr8'.")
parser.add_argument("-p", "--pathotype_book", required=True, help="Absolute path to pathotype book.")
parser.add_argument("-L", "--library", required=True, help="Absolute path to kmerGWAS library.")
parser.add_argument("-O", "--outdir", required=True, help="Absolute path to the directory where kmerGWAS output is generated.")
parser.add_argument("-S", "--strand_dir", required=True, help="Absolute path to sample strands.")
parser.add_argument("-r", "--refHetfilt",  default="True", help="(Optional) Filter for PST accessions based on refHetFreqSpec. Can be either True/False. Default = True.")
parser.add_argument("--mac", type=int, default=2, help="(Optional) Minor allele count (minimum allowed appearance of a k-mer). Default = 2.")
parser.add_argument("-m", "--min_per", type=float, default=0.1, help="(Optional) Minimum percent of appearance in each strand form. Default = 0.1.")
parser.add_argument("-k", "--kmer_length", type=int, default=31, help="(Optional) kmer length in bp. Default = 31.")
parser.add_argument("-t", "--threads", help="(Optional) Number of threads. Default = 8.")
parser.add_argument("--random_lev", required=True, type=float, help="Proportion of pathotype to be permuted.")
...
# ============= pathotype permutation ============= #

def pathotype_permutation(pheno_df, random_level):
    assert type(random_level) == float, "Randomisation level must be float"
    assert random_level >= 0 and random_level <= 1.0, "Randomisation level must be within range 0.0 - 1.0"
    if random_level == 0.0:
        return pheno_df
    else:
        subset = pheno_df.sample(math.ceil(random_level*len(pheno_df))).sort_index()
        pheno_df = pd.DataFrame([pheno_df.iloc[i] for i in pheno_df.index if i not in subset.index])
        subset.phenotype_value = random.sample(list(subset.phenotype_value), k=len(subset))
        return pheno_df.append(subset).sort_index()

def get_perm_phenotype_value(pb_fn, Yr, outpath, random_level, refHet_filt='True'):
    ...
    for SRA in inclusion_list:
        ...
        pheno_df = pd.DataFrame(pheno_val_list, columns=['accession_id','phenotype_value'])
        perm_pheno_df = pathotype_permutation(pheno_df, random_level)
        output_fn = os.path.join(outpath, f"Yr{Yr}_phenotype_values_perm{random_level}.tsv")
        pd.DataFrame(perm_pheno_df.to_csv(output_fn,sep='\t',index=False))
        
        
        
# ========= (II) kmerGWAS ========= #       

def create_date_Yr_fn(Yr_name, date, pb_fn, random_level, refHet_filt='True'):
    """
    ...
    """
    TODAY_OUTDIR = os.path.join(OUTDIR, date, f"perm_{random_level}", Yr_name)
    print(TODAY_OUTDIR)
    if not os.path.exists(TODAY_OUTDIR):
        os.makedirs(TODAY_OUTDIR)
    
    os.mkdir(f"{TODAY_OUTDIR}/{Yr_name}_perm{random_level}_phenoTable")
    get_perm_phenotype_value(pb_fn, Yr_name[2:], f"{TODAY_OUTDIR}/{Yr_name}_perm{random_level}_phenoTable", random_level, refHet_filt)
    return TODAY_OUTDIR
...
def kmerGWAS_for_Yr(Yr_name, date, pb_fn, LIB_DIR, OUTBASE_DIR, STRAND, random_level, refHet_filt='True', X=2, P=0.1, K=31, THREADS=8):
    global STRAND_DIR
    STRAND_DIR = STRAND
    global OUTDIR
    OUTDIR = OUTBASE_DIR
    global TODAY_OUTDIR
    TODAY_OUTDIR = create_date_Yr_fn(Yr_name, date, pb_fn, random_level, refHet_filt)
    inclu_list = pd.read_csv(f"{TODAY_OUTDIR}/{Yr_name}_perm{random_level}_phenoTable/{Yr_name}_phenotype_values_perm{random_level}.tsv",sep='\t').iloc[:,0].tolist()
    pheno_val_path = f"{TODAY_OUTDIR}/{Yr_name}_perm{random_level}_phenoTable/{Yr_name}_phenotype_values_perm{random_level}.tsv"
    run_kmer_GWAS(kmers_list_paths(inclu_list), pheno_val_path, LIB_DIR, OUTDIR, X, P, K, THREADS)
    


if __name__ == "__main__":
    kmerGWAS_for_Yr(args.host_strain_name, args.date, args.pathotype_book, args.library, args.outdir, args.strand_dir, args.random_lev, args.refHetfilt, args.mac, args.min_per, args.kmer_length, args.threads)